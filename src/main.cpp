...
const char *mqtt_client = "mqtt_client-001";
...
void loop()
{
  static uint8_t countWork = 10;
  if ((WiFi.getMode() == WIFI_STA) && (WiFi.status() != WL_CONNECTED))
  {
    (setupWiFi()) ? countWork = 10 : countWork--;
  }

  if ((countWork == 10) && (!client.connected()))
  {
    if (reconnect())
    {
      countWork = 10;
  ...
  if (Serial.available())
  {
    ...
    if (bufString.substring(0, 2).toInt() == sensorNumRoom)
    {
